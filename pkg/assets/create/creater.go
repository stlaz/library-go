...
func create(ctx context.Context, manifests map[string]*unstructured.Unstructured, client dynamic.Interface, mapper meta.RESTMapper, options CreateOptions) (error, bool) {
	...
	for _, path := range sortedManifestPaths {
		...
		incluster, err := resource.Create(context.TODO(), manifests[path], metav1.CreateOptions{})
		...
		if kerrors.IsAlreadyExists(err) {
			...
			incluster, err = resource.Get(context.TODO(), manifests[path].GetName(), metav1.GetOptions{})
			if err != nil {
				if options.Verbose {
		...
		if _, ok := manifests[path].Object["status"]; ok {
			...
			if !found {
				...
				incluster, err = resource.UpdateStatus(context.TODO(), incluster, metav1.UpdateOptions{})
